 ...
 *
 */
typedef unsigned (*ucg_collective_progress_t)(void *op);



typedef void* ucg_message_pool_h;

/**
 * @ingroup UCG_GROUP
 * @brief Callback function to indicate the collective operations was completed.
 *
 * This routine, provided by the application, will be called by UCX once all the
 * requirements for this collective operation have been satisfied. This does not
 * necessarily mean that other (remote) peers have also completed the operation
 * on their end.
 *
 * @param [in]  req          Request object passed to @ref ucg_collective_start .
 * @param [in]  status       Indication of the status of the complete operation.
 *
 */
typedef void (*ucg_collective_comp_cb_t)(void *req, ucs_status_t status);


/**
 * @ingroup UCG_GROUP
 * @brief Callback function to indicate the latest measured rank imbalance time.
 *
 * This routine, provided by the application, will be called by UCX whenever new
 * information about the imbalance between processes becomes available. In this
 * context, imbalance is the time difference (in seconds) between the first
 * process in the groupd joining the collective operation and the completion of
 * said operation on this process. This value may vary among processes in the
 * same operation, and provides a lower bound on the imbalance across the group.
 *
 * @param [in]  cb_context   Context object passed to @ref ucg_group_create .
 * @param [in]  imbalance    A lower bound on the time between callers starting.
 *
 */
typedef void (*ucg_collective_imbalance_set_cb_t)(void *cb_context,
                                                  double imbalance);
