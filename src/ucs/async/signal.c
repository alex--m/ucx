...
#define UCS_SIGNAL_TIMER_IDX_SHIFT (24)
...
static ucs_status_t ucs_async_signal_dispatch_timer(int uid)
{
    ...
    return ucs_async_dispatch_timerq(&timer->timerq, ucs_get_time(),
                                     uid << UCS_SIGNAL_TIMER_IDX_SHIFT);
}
...
ucs_async_signal_timerq_add_timer(ucs_async_signal_timer_t *timer, int tid,
                                  ucs_time_t interval, unsigned *timer_id_p)
{
    int uid = timer - ucs_async_signal_global_context.timers;
    ...
    ucs_assert_always(*timer_id_p <= UCS_MASK(UCS_SIGNAL_TIMER_IDX_SHIFT));
    *timer_id_p = (uid << UCS_SIGNAL_TIMER_IDX_SHIFT) |
                  (*timer_id_p & UCS_MASK(UCS_SIGNAL_TIMER_IDX_SHIFT));
                                     ...
                                     unsigned timer_id)
{
    ...
    ucs_assert((timer_id >> UCS_SIGNAL_TIMER_IDX_SHIFT) ==
               (timer - ucs_async_signal_global_context.timers));

    timer_id &= UCS_MASK(UCS_SIGNAL_TIMER_IDX_SHIFT);
                                               ...
                                               unsigned *timer_id_p)
{
    ...
    if (timer == NULL) {
        ...
        status = ucs_async_signal_timerq_add_timer(timer, tid, interval,
                                                   timer_id_p);
    }
                                                  ...
                                                  unsigned timer_id)
{
    ...
    ucs_trace_func("async=%p timer_id=%u", async, timer_id);
