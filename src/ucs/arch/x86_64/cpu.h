...
#define UCS_ARCH_SHARE_CACHE_MASK (UCS_CPU_FLAG_CLWB | UCS_CPU_FLAG_CLDEMOTE)
...
static inline void ucs_arch_share_cache(void *addr)
{
     ...
     */
    switch (g_supported_cpu_flags & UCS_ARCH_SHARE_CACHE_MASK) {
#if __CLDEMOTE__
    case UCS_CPU_FLAG_CLWB | UCS_CPU_FLAG_CLDEMOTE:
    case UCS_CPU_FLAG_CLDEMOTE:
        _mm_cldemote(addr);
        break;
#endif
#if __CLWB__
    case UCS_CPU_FLAG_CLWB:
        _mm_clwb(addr);
        ucs_prefetch_write(addr);
        break;
#endif
    default:
        break;
