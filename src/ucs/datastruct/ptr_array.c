...
void ucs_ptr_array_init(ucs_ptr_array_t *ptr_array, const char *name)
{
    ...
    if (name != NULL) {
        ptr_array->start = (ucs_ptr_array_elem_t*)ucs_strdup(name,
                                                             "ptr_array_name");
    }
#endif
}
...
static void ucs_ptr_array_grow(ucs_ptr_array_t *ptr_array, unsigned new_size)
{
    ...
    if (ptr_array->size == 0) {
        name             = (char*)ptr_array->start;
        ptr_array->start = NULL;
    } else {
        name = UCS_MEMTRACK_NAME_REUSE;
    }
#endif

    /* Allocate new array */
    new_array = ucs_realloc(ptr_array->start,
                            new_size * sizeof(ucs_ptr_array_elem_t)
                            UCS_MEMTRACK_NAME(name));
    ...
    if (ptr_array->freelist == UCS_PTR_ARRAY_SENTINEL) {
        ...
        do {
            i = next;
            ucs_assert(i < curr_size);
            ucs_assert(__ucs_ptr_array_is_free(new_array[i]));
