                            ...
                            unsigned element_index, unsigned flags, void **var)
{
    ...
    ret = ucs_ptr_array_lookup(&locked_ptr_array->super, element_index, *var);

    if ((flags & UCS_PTR_ARRAY_LOCKED_FLAG_KEEP_LOCK_IF_FOUND) && ret) {
        return ret;
    }

    if ((flags & UCS_PTR_ARRAY_LOCKED_FLAG_KEEP_LOCK_IF_NOT_FOUND) && !ret) {
        return ret;
    }
