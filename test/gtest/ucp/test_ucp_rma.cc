...
public:
    ...
    void append_b(size_t size, void *expected_data, ucp_mem_h memh,
                  void *target_ptr, ucp_rkey_h rkey, void *arg)
    {
        /* ucs_memory_type_t args[] = {send_mem_type, target_mem_type}; */
        ucs_status_ptr_t status_ptr = do_append(size, expected_data, memh,
                                                target_ptr, rkey, arg);
        request_wait(status_ptr);
    }

    void append_nbi(size_t size, void *expected_data, ucp_mem_h memh,
                 void *target_ptr, ucp_rkey_h rkey, void *arg)
    {
        /* ucs_memory_type_t args[] = {send_mem_type, target_mem_type}; */
        ucs_status_ptr_t status_ptr = do_append(size, expected_data, memh,
...
private:
    ...
    ucs_status_ptr_t do_append(size_t size, void *expected_data, ucp_mem_h memh,
                               void *target_ptr, ucp_rkey_h rkey, void *arg)
    {
        uint64_t *result;
...
UCS_TEST_P(test_ucp_rma, get_blocking_zcopy, "ZCOPY_THRESH=0") {
    ...
    test_mem_types(static_cast<send_func_t>(&test_ucp_rma::get_b), 128, false,
                   64 * UCS_KBYTE);
}
