...
protected:
    virtual void init() {
        ...
        modify_config("INSTRUMENT_TYPES", "tx");
...
static void test_func(uint32_t *location_id)
{
    *location_id = ucs_instrument_register((ucs_instrumentation_types_t)0,
                                             "test") + 1;
...
UCS_TEST_F(instrument, record) {
    ...
    ASSERT_EQ((uint32_t)2, hdr.num_locations);
    ...
    EXPECT_GE(record.location, location_id);
    EXPECT_EQ(location.location, record.location);
    UCS_TEST_MESSAGE << "Location name is: " << location.name;
}
